<div>
    ...
    <x-datatable :items="$archives" searchPlaceholder="Cari SK/dokumen..." emptyMessage="Tidak ada data SK/Lainnya">
        ...
        <x-slot:columns>
            ...
            <x-datatable.th center>Jenis</x-datatable.th>
        ...
        @forelse ($archives as $index => $archive)
            ...
            <x-datatable.row>
                ...
                <x-datatable.cell center>
                    <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full {{ $jenisBadge['bg'] }} {{ $jenisBadge['text'] }}">{{ $jenisBadge['label'] }}</span>
                </x-datatable.cell>
                <x-datatable.cell center>
                    @if($archive->category)
