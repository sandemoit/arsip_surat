<div>
    ...
    <div class="mb-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-zinc-900">Data SK / Lainnya</h1>
                <p class="text-md text-zinc-500 mt-1">Kelola arsip SK dan dokumen lainnya</p>
            </div>
            <div class="flex gap-2">
                <flux:button variant="ghost" icon="arrow-down-tray">Export</flux:button>
                <flux:button icon="plus" href="{{ route('arsip.upload') }}" class="bg-green-500 text-white hover:bg-green-600">Tambah Arsip</flux:button>
            </div>
        </div>
    </div>
    ...
    <x-datatable :items="$archives" searchPlaceholder="Cari SK/dokumen..." emptyMessage="Tidak ada data SK/Lainnya">
        <x-slot:filters>
        ...
        <x-slot:columns>
            ...
            <x-datatable.th center>Diupload</x-datatable.th>
        ...
        @forelse ($archives as $index => $archive)
            ...
            <x-datatable.row>
                ...
                <x-datatable.cell center>
                    @if($archive->category)
                        <x-datatable.badge color="purple">{{ $archive->category->name }}</x-datatable.badge>
                    @else <span class="text-zinc-400">-</span> @endif
                </x-datatable.cell>
                <x-datatable.cell center>
                    <span class="text-sm text-zinc-600">{{ $archive->uploader->name ?? '-' }}</span>
                </x-datatable.cell>
                <x-datatable.cell center>
